...
for (i in 1:72) {
  boxplot[[i]] = ggplot()+
                  geom_boxplot(aes(x = result_AB_list[[weight_to_plot]][i,]))+
  labs(title = paste0("Box plot for AB_hat - AB, entry = ", paste(get_index(i)[1], sep = ", ", get_index(i)[2])))
}
...
index1 <- order(abs_mean_entry_bias)
index2 <- order(sd_entry_bias)

for (j in 1:10) {
  print(boxplot[[index1[j]]])
  # boxplot[[index2[j]]]
}
...
ggplot()+
  geom_boxplot(aes(x = result_AB_list[[weight_to_plot]][70,]))
