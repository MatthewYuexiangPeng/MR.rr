...
A_bias <- ggplot(long_data, aes(x = Category, y = Values)) + geom_boxplot() + labs(title = "Boxplot for Each weight", x = "weight", y = "A_hat-A norm") + theme_minimal()
A_bias
...
B_bias <- ggplot(long_data, aes(x = Category, y = Values)) + geom_boxplot() + labs(title = "Boxplot for Each weight", x = "weight", y = "B_hat-B norm") + theme_minimal()
B_bias
