...
.simulation_sparse_compare <- function(parameters, lambda = rep(2e-3, parameters$px), r_RR = 2, pz) {
  n <- pz
  py <- parameters$py
  px <- parameters$px
  C <- parameters$C
...
# Sparse penalty for each simulation setting
sparse_lambda_list <- list(
  "me_2.5_effect_0.25" = rep(1e-4, px),
  "me_2.5_effect_1"    = rep(1e-3, px),
  "me_1_effect_0.25"   = rep(1e-3, px),
  "me_1_effect_1"      = rep(1e-3, px)
)

# Regularization rates for Reg. MR-rr
# Keep these consistent with the corresponding main simulation
regularization_rate_list <- c(
...
run_simulation_sparse_compare <- function(C, B_sparse, lambda = rep(1e-3, ncol(C)), r_RR = 2, eloop = 100, pz) {
  py <- nrow(C)
  px <- ncol(C)
  ...
  parameters <- .get_parameters(C, me_weight = 1, effect_weight = 1, r_RR = r_RR, pz)
  # B_sparse <- parameters$B  # true sparse B
  B_true_vec <- as.numeric(B_sparse != 0)  # logical vector
  
  for (i in 1:eloop) {
    sim <- .simulation_sparse_compare(parameters = parameters, lambda = lambda, r_RR = r_RR, pz)
...
set.seed(123)
res = run_simulation_sparse_compare(C, B_sparse, lambda = rep(1e-3, 9), eloop = 1000, pz)
