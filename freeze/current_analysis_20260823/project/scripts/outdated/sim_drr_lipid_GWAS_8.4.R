...
get_parameters = function(sample_weight, r_RR = 5, px,  r_approx = 5){
  ...
  # Question: still use the previous var z? 
  ...
  z_index = sample(1:1119, pz, replace = TRUE)
  var_Z = var_Z[z_index]
  ...
  Sigma_Xj = lapply(1:pz_lip_data, function(j) diag(sigma_gamma_j[j,]) %*% Sigma %*% diag(sigma_gamma_j[j,]))
  ...
  var_Y = sample(lip_data$se_out1[sample(1:114, 1119, replace = TRUE)]^2 * bmi.cad$N.outcome * var_Z_raw, py) # Var(Y_k) can be in the range of this (although looks strange probably because the coef is from logistic model, but I ignore this for now)
...
simulation = function(parameters, n, penalized = FALSE) {
  ...
  if (penalized == FALSE) {
    ...
    result_d <- debiased_RRR_modified(y_j_hat, x_j_hat, r=r_RR, sqrt_Gamma = weight.matrix, Sigma_X = Sigma_X, ph=regularization_rate)
  }
  A_d_hat = result_d$A
  B_d_hat = result_d$B
...
for (i in 1:(px*py)) {
  abs_mean_entry_bias_d[i] <- abs(mean(result_AB_d_list[[weight_to_plot]][i,])) # TODO: abs_mean_entry_bias_d and result_AB_d_list inconsistent. Due to abs
...
min(eigen(parameters$VX_tilde)$values)/min(eigen(parameters$Sigma_X)$values)
...
# result_AB_d_list
mean(as.matrix(as.numeric(mean_entry_bias_d)))
mean(as.matrix(mean_entry_bias))
...
for (loop in 1:iteration){
  ...
  for (i in 1:bootstrap_size){
    ...
    if (penalized == FALSE) {
      ...
      result_d_bt <- debiased_RRR_modified(y_j_hat_star, x_j_hat_star, r=5, sqrt_Gamma = solve(Sigma_Y_hat), Sigma_X = Sigma_X_hat, ph=regularization_rate)
    }
