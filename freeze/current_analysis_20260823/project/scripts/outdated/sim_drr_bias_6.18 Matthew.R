...
get_parameters = function(sample_weight, r_RR = 3, px,  r_approx = 3){
  ...
  #                c(rep(0, 6), rnorm(2)))
  # temp.B = rbind(c(rnorm(3), rep(0, 47)),
  #                c(rep(0, 3), rnorm(3), rep(0, 44)),
  #                c(rep(0, 6), rnorm(3), rep(0, 41)))
  # C <- temp.A %*% temp.B
  # eigen(C%*%t(C))
  # qr(C)$rank
  
  # C = matrix(rnorm(r_RR*py), py, r_RR) %*% matrix(rnorm(r_RR*px), r_RR, px)
  
  # C = cbind(diag(c(rep(1,3),rep(0,py-3))), matrix(0, py, px-3))
  ...
  parameters = list(py = py, px = px, var_Z = var_Z, VX_tilde = VX_tilde_upscale, 
                    ...
                    VY_tilde = VY_tilde_upscale, weight.matrix = weight.matrix)
  return(parameters)
}
...
simulation = function(parameters, n) {
  ...
  gamma_j_star = mvrnorm(n = n, mu = rep(0, px), Sigma = VX_tilde, tol = 100) # xj 
  Gamma_j_star = gamma_j_star %*% t(C) # yj
  ...
  return(list(A_hat, B_hat, AB_hat, A_d_hat, B_d_hat, AB_d_hat, xj = t(gamma_j_star), yj_hat = t(y_j_hat)))
}
...
for (sample_weight in sample_weight_list) {
  ...
  simulation_pz = 1000
  for (i in 1:eloop) {
    simulation_result = simulation(parameters, n=simulation_pz)
    ...
    # use loss function in THM 2.1/ 2.2 to investigate bias
    # xj = simulation_result[[7]]
    # yj_hat = simulation_result[[8]]
    # squared_loss = 1/simulation_pz * sqrt_matrix(weight.matrix) %*% (yj_hat - AB_hat %*% xj) %*% t(yj_hat - AB_hat %*% xj) %*% sqrt_matrix(weight.matrix)
    # squared_loss_d = 1/simulation_pz * sqrt_matrix(weight.matrix) %*% (yj_hat - AB_d_hat %*% xj) %*% t(yj_hat - AB_d_hat %*% xj) %*% sqrt_matrix(weight.matrix)
    # Loss_THM22 = sum(diag(squared_loss))
    # Loss_THM22_d = sum(diag(squared_loss_d))
  }
...
weight_to_plot <- "5" # choose the weight to plot for AB
...
combined_plot <- myplots_d[[index1_d[5]]] + myplots_d[[index1_d[4]]] +
  ...
  plot_annotation(
    title = "Top 5 largest entries of debiased AB_hat - C^(r) and AB_hat - C^(r) entry bias",
    subtitle = "C^(r) ",
    tag_levels = 'A',
