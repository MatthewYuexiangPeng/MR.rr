...
#### bootstrap ----
bootstrap_loop = 100  # used to calculate coverage rate in all loops
bootstrap_size = 100  # sample size to calculate CI in each bt loop
# note: finally we will average the entry-wise coverage rate, suppose to be about 95%
pz = 1000
...
for (i in 1:bootstrap_size){
  ...
  if (panalized == FALSE) {
    ...
    result_d_bt <- debiased_RRR_modified(y_j_hat_star, x_j_hat_star, r=5, sqrt_Gamma = solve(Sigma_Y_hat), Sigma_X = Sigma_X_hat, ph=0.00001)
  }
...
# way 1 to see coverage: see whether the true C is in the 95% percentile interval ----
# 95% percentile intervale of each vecor in C_drr_hat_entrywise_bt_list
C_drr_hat_entrywise_bt_95_list = list()
AB_d_hat_vec = as.vector(AB_d_hat)
for (i in 1:(px*py)){
  C_drr_hat_entrywise_bt_95_list[[i]] = quantile(C_drr_hat_entrywise_bt_list[[i]], c(0.05, 0.95))
}
# see whether the true C is in the 95% percentile interval
...
# way 2 to see coverage -----
# compuete variance for each entry
C_drr_hat_entrywise_bt_var_list = list()
for (i in 1:(px*py)){
  C_drr_hat_entrywise_bt_var_list[[i]] = var(C_drr_hat_entrywise_bt_list[[i]])
}
# CI for each entry
C_drr_hat_entrywise_bt_CI_list = list()
for (i in 1:(px*py)){
  C_drr_hat_entrywise_bt_CI_list[[i]] = c(AB_d_hat_vec[i] - 1.96 * sqrt(C_drr_hat_entrywise_bt_var_list[[i]]), AB_d_hat_vec[i] + 1.96 * sqrt(C_drr_hat_entrywise_bt_var_list[[i]]))
}
# see whether the true C is in the 95% percentile interval
index_in95 = rep(1, px*py)
for (i in 1:(px*py)){
  if (C_r_vec[i] < C_drr_hat_entrywise_bt_CI_list[[i]][1] | C_r_vec[i] > C_drr_hat_entrywise_bt_CI_list[[i]][2]){
    index_in95[i] = 0
  }
}
mean(index_in95)
...
# method 2: non-parametric bootstrap -----
bt_sample_number = 5000
C_drr_hat_entrywise_bt_list_2 = list()
for (i in 1:(px*py)){
  C_drr_hat_entrywise_bt_list_2[[i]] = rep(0, bootstrap_size)
}
for (i in 1:bootstrap_size){
  # sample bt_sample_number rows from x_j_hat, y_j_hat
  index_bt = sample(1:pz, bt_sample_number, replace = TRUE)
  x_j_hat_star_2 = x_j_hat[index_bt,]
  y_j_hat_star_2 = y_j_hat[index_bt,]
  
  # estimate C_drr_hat
  if (panalized == FALSE) {
    result_d_bt_2 <- debiased_RRR(y_j_hat_star_2, x_j_hat_star_2, r=5, sqrt_Gamma = solve(Sigma_Y_hat), Sigma_X = Sigma_X_hat)
  } else {
    result_d_bt_2 <- debiased_RRR_modified(y_j_hat_star_2, x_j_hat_star_2, r=5, sqrt_Gamma = solve(Sigma_Y_hat), Sigma_X = Sigma_X_hat, ph=0.00001)
  }
  result_d_bt_vec_2 = as.vector(result_d_bt_2$AB)
  for (j in 1:(px*py)){
    C_drr_hat_entrywise_bt_list_2[[j]][i] = result_d_bt_vec_2[j]
  }
}
# way 1 to see coverage ----
# 95% percentile intervale of each vecor in C_drr_hat_entrywise_bt_list
C_drr_hat_entrywise_bt_95_list_2 = list()
for (i in 1:(px*py)){
  C_drr_hat_entrywise_bt_95_list_2[[i]] = quantile(C_drr_hat_entrywise_bt_list_2[[i]], c(0.05, 0.95))
}
# see whether the true C is in the 95% percentile interval
index_in95_2 = rep(1, px*py)
for (i in 1:(px*py)){
  if (C_r_vec[i] < C_drr_hat_entrywise_bt_95_list_2[[i]][1] | C_r_vec[i] > C_drr_hat_entrywise_bt_95_list_2[[i]][2]){
    index_in95_2[i] = 0
  }
}
mean(index_in95_2)
...
# way 2 to see coverage -----
# compuete variance for each entry
C_drr_hat_entrywise_bt_var_list_2 = list()
for (i in 1:(px*py)){
  C_drr_hat_entrywise_bt_var_list_2[[i]] = var(C_drr_hat_entrywise_bt_list_2[[i]])
}
# CI for each entry
C_drr_hat_entrywise_bt_CI_list_2 = list()
for (i in 1:(px*py)){
  C_drr_hat_entrywise_bt_CI_list_2[[i]] = c(AB_d_hat_vec[i] - 1.96 * sqrt(C_drr_hat_entrywise_bt_var_list_2[[i]]), AB_d_hat_vec[i] + 1.96 * sqrt(C_drr_hat_entrywise_bt_var_list_2[[i]]))
}
# see whether the true C is in the 95% percentile interval
index_in95_2 = rep(1, px*py)
for (i in 1:(px*py)){
  if (C_r_vec[i] < C_drr_hat_entrywise_bt_CI_list_2[[i]][1] | C_r_vec[i] > C_drr_hat_entrywise_bt_CI_list_2[[i]][2]){
    index_in95_2[i] = 0
  }
}
mean(index_in95_2)
...
# the following rate represent the proportion of entries of true C^r that be covered by 95% bootstrap C^r_hat

## penalized estimator: 
### parametric bootstrap:
#### percentile coverage: 0.45
#### CI coverage: 0.4675
### non-parametric bootstrap:
#### percentile coverage: 0.275
#### CI coverage: 0.2875

## un-penalized estimator: (variance very large, so iinterval can cover easily)
### parametric bootstrap: 
#### percentile coverage: 1
#### CI coverage: 1
### non-parametric bootstrap:
#### percentile coverage: 1
#### CI coverage: 1
