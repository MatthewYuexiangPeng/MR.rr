...
#### 260419 parallel bootstrap - no rank test, auto regularization_rate ####
nonpara_bootstrap_parallel <- function(parameters_list, me_weight = "1", effect_weight = "1",
                                       regularization_rate = NULL,  # NULL = auto from list
                                       bootstrap_size = 500,
  ...
  # ===== Auto-pick regularization_rate to match main simulation =====
  if (is.null(regularization_rate)) {
    if (!exists("regularization_rate_list")) {
      stop("regularization_rate_list not found in global env. ",
           "Either define it or pass regularization_rate explicitly.")
    }
  ...
  cat(sprintf("ME weight: %s, Effect weight: %s (setting index %d)\n",
              me_weight, effect_weight, param_index))
  cat(sprintf("Regularization rate: %.6e\n", regularization_rate))
  cat(sprintf("Rank (fixed): %d\n", r_rank))
  ...
  run_one_iter <- function(loop_id) {
    # generate data only (no rank test, no estimators inside)
    sim     = .simulation_data_only(parameters)
    x_j_hat = sim$x_j_hat
    y_j_hat = sim$y_j_hat
    
    # storage: rows = bootstrap draws, cols = entries of C
  ...
  list(
    setting           = list(me_weight = me_weight,
...
# ============================================================ #
set.seed(123)
test_sim_pred_filename = "results/simulate_result_pred_260416_1000.RData"
load(test_sim_pred_filename)


# test
result_test <- nonpara_bootstrap_parallel(
...
result <- nonpara_bootstrap_parallel(
  ...
  me_weight       = "1",
  effect_weight   = "1",
  regularization_rate = NULL,   # auto-pick from regularization_rate_list
  bootstrap_size  = 100,
  iteration       = 1000,
  n_cores         = 10,
  estimator_set   = "no_mrdag",
...
save(result, file = "results/CI_coverage_parallel_no_mrdag_260419_1_1.RData")


#### 260419 MrDAG-only parallel bootstrap (aligned with no_mrdag version) ####
nonpara_bootstrap_parallel_mrdag <- function(parameters_list, me_weight = "1", effect_weight = "1",
                                             iteration = 300, bootstrap_size = 100,
                                             r_rank = 2,          # kept for API consistency (unused by MrDAG)
                                             n_cores = NULL,
  ...
  estimator_order = c("MrDAG")   # fixed for this function
  
  start_time = Sys.time()
  cat(sprintf("\n========================================\n"))
  cat(sprintf("Starting MrDAG-only Parallel Bootstrap\n"))
  cat(sprintf("========================================\n"))
  cat(sprintf("ME weight: %s, Effect weight: %s (setting index %d)\n",
              me_weight, effect_weight, param_index))
  cat(sprintf("MrDAG: niter=%d, burnin=%d\n", mrdag_niter, mrdag_burnin))
  cat(sprintf("Iterations: %d, Bootstrap size: %d\n", iteration, bootstrap_size))
  cat(sprintf("Cores: %d\n", n_cores))
  cat(sprintf("Started at: %s\n", format(start_time, "%Y-%m-%d %H:%M:%S")))
  cat(sprintf("========================================\n\n"))
  ...
  run_one_iter <- function(loop_id) {
    # 用和 no_mrdag 完全一样的数据生成器
    sim     = .simulation_data_only(parameters)
    x_j_hat = sim$x_j_hat
    y_j_hat = sim$y_j_hat
    
    # 存 bootstrap 估计值：行 = bootstrap draws, 列 = C 的 entries
    ...
    for (bt in 1:bootstrap_size) {
      ...
      est_mat <- tryCatch(
        ...
        error = function(e) { warning(sprintf("MrDAG failed at bt=%d: %s", bt, e$message));
          matrix(NA_real_, py, px) }
      )
      draws_mat[bt, ] = as.vector(est_mat)
    }
    
    # ===== CI / coverage（和 no_mrdag 返回结构一致）=====
    ci <- apply(draws_mat, 2, quantile,
                probs = c(0.025, 0.975), na.rm = TRUE, names = FALSE)
    
    list(
      coverage  = list(MrDAG = as.numeric(C_vec >= ci[1, ] & C_vec <= ci[2, ])),
  ...
  cat(sprintf("\n========================================\n"))
  cat(sprintf("MrDAG Bootstrap completed!\n"))
  cat(sprintf("Total time: %s\n", fmt(total_time)))
  cat(sprintf("Completed at: %s\n", format(Sys.time(), "%Y-%m-%d %H:%M:%S")))
  cat(sprintf("========================================\n\n"))
  flush.console()
  
  # ===== Aggregate（和 no_mrdag 格式完全一致）=====
  coverage_list = lapply(estimator_order, function(name) {
    mat = sapply(coverage_results, function(r) as.numeric(r$coverage[[name]]))
    if (is.vector(mat)) mat = matrix(mat, nrow = 1)
    else if (nrow(mat) != d) mat = t(mat)
...
# test
result_test <- nonpara_bootstrap_parallel_mrdag(
  parameters_list = simulate_result_prediction$parameters_list,
  me_weight = "1", effect_weight = "1",
  iteration = 3, bootstrap_size = 5,   # 极小
  n_cores = 4,
  mrdag_niter = 200, mrdag_burnin = 50,
  save_estimates = FALSE
)


# formal
set.seed(124)   # 和 no_mrdag 不同 seed，避免碰撞
result_mrdag <- nonpara_bootstrap_parallel_mrdag(
  parameters_list = simulate_result_prediction$parameters_list,
  me_weight       = "1",
  effect_weight   = "1",
  iteration       = 1000,      # MrDAG 太慢，比 no_mrdag 少 3x
  bootstrap_size  = 100,
  n_cores         = 16,
  mrdag_niter     = 1000,
  mrdag_burnin    = 200,
...
save(result_mrdag, file = "results/CI_coverage_mrdag_only_260419_1_1.RData")
...
load("results/CI_coverage_parallel_no_mrdag_260419_1_1.RData")
