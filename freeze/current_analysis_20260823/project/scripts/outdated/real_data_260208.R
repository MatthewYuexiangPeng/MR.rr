...
rownames(A_MRrr_regularized) <- paste0("out", 1:3)
...
draw_multiple_heatmaps(
  ...
  filename = "results/Causal_C_estimation_hm_260208.png",
  ncol = 3
)
...
save(bootstrap_C_list, file = "bootstrap_C_list_1000_260208.RData")
# load bootstrap_C_list
load("bootstrap_C_list_1000_260208.RData")
...
#                  save_path = "results/estimator_realdata_with_CI_90_260208.png")
...
plot_realdata_halfeye_old <- function(summary_data, out_name, exp_name,
                                  title = "Point estimates and 95% non-parametric bootstrap confidence intervals for real data",
                                  save_path = NULL,
                                  width = 12, height = 6,
                                  ylim = c(-1, 1)) {
...
plot_realdata_halfeye(summary_data_long, out_name, exp_name,
                      ylim = c(-0.5, 0.5),
                      save_path = "results/real_data_CI_halfeye_260208_test.png")
