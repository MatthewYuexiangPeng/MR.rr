...
Sigma_Yj = lapply(1:pz_lip_data, function(j)
  diag(sigma_Gamma_j[j,]) %*% Corr_Y %*% diag(sigma_Gamma_j[j,]))
# Sigma_Y_temp = lapply(1:pz, function(j) Sigma_Yj[[j]])
...
#### Single simulation run for sparse MR-RR ####
...
#### Run multiple simulations for sparse MR-RR ####
...
# save
save(res, file = "results/simulation_sparse_result_250708.RData")
load("results/simulation_sparse_result_250708.RData")
...
plot_sparse_B_estimation <- function(res, 
  ...
  B_hat_matrix <- res$result_sparse_B   # r * px × N
