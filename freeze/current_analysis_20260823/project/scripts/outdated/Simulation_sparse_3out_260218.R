...
#### 260222 new ver ####
run_simulation_sparse_compare <- function(C, B_sparse, lambda = rep(1e-4, ncol(C)), r_RR = 2, eloop = 100) {
  py <- nrow(C)
  px <- ncol(C)
...
res = run_simulation_sparse_compare(C, B_sparse, lambda = rep(5e-5, 9), eloop = 1000)

# save
save(res, file = "results/simulation_sparse_result_060223_5e-5.RData")
load("results/simulation_sparse_result_060223_5e-5.RData")
...
plot_sparse_B_estimation_canonicalized <- function(res, 
  ...
  # ---------------------------
  # 1) canonicalize sign by row (your function)
  B_true_can <- canonicalize_B(B_true)
  
  # 2) get a fixed row order from B_true_can (same rule as canonicalize_B_with_order)
  ...
  for (n in 1:N) {
    Bn <- matrix(B_hat_matrix[, n], nrow = r, ncol = px)  # column-major => matches as.vector(B)
    Bn <- canonicalize_B(Bn)                               # sign canonicalize
    Bn <- Bn[ord_true, , drop = FALSE]                     # row order aligned to B_true
    B_hat_can[, n] <- as.vector(Bn)                        # back to (r*px) vector (column-major)
  }
  B_hat_matrix <- B_hat_can
  B_true <- B_true_can
  
  # ---------------------------
  # Build plotting data (same as your original)
  # ---------------------------
  ...
  # label helper (avoid hard-coding r=2)
  ...
  p <- ggplot(df_long, aes(x = "", y = Value)) +
    ...
    facet_grid(
      rows = vars(Row),
      cols = vars(Col),
      labeller = labeller(Row = row_lab, Col = exp_name),
      scales = "free"
    ) +
    theme_bw(base_size = 10) +
    ...
    xlab("") +
    ggtitle("Simulation Boxplots of Estimated B Entries (Canonicalized)\n(Red = Nonzero True B, Gray = Zero True B)")
