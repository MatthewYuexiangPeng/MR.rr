...
rank_test = function(W, y_j_hat, x_j_hat, Sigma_X, print = TRUE, min_rank = 1){
  px = ncol(x_j_hat)
  py = ncol(y_j_hat)
  pz = nrow(y_j_hat)
...
save_sim_filename = "results/simulate_result_pred_250702.RData"
...
run_simulation_prediction <- function(regularization_rate_list){
  ...
  result_AB_list = result_AB_d_list = result_AB_d_r_list = 
    ...
    # rank correct
    rank_correct_list = template_list
  ...
  for (me_weight_index in seq_along(me_weight_list)){
    ...
    for (effect_weight_index in seq_along(effect_weight_list)){
      ...
      for (i in 1:eloop) {
        ...
        # if rank correct
        rank_correct[i] = simulation_result[[length(simulation_result)]]
...
save(simulate_result_prediction, file = save_sim_filename)
...
plot_simulation_result_pred(sim_result = simulate_result_prediction,
                                        me_weight = me_weight_list[2], 
                                        effect_weight = effect_weight_list[2],
                                        Y = Y, 
                                        type = "box", 
                                        width = 12, height = 6)
