...
# before 3.23 add debias:
# regularization_rate_list = c(1.279385e-11, 1.913728e-12, 3.941866e-13, 6.660968e-14, 3.191639e-15)

# [1] 3.065489e-11 2.491197e-12 1.252192e-14 8.756005e-16 1.432915e-17
regularization_rate_list = c(3.065489e-11, 2.491197e-12, 1.252192e-14, 8.756005e-16, 1.432915e-17)
...
run_simulation <- function(regularized = TRUE, regularization_rate_list){
  sample_weight_list = c(1, 0.5, 0.2, 0.15, 0.1)
  eloop = 30
  result_AB_list = list("1" = NA, "0.5" = NA, "0.2" = NA, "0.15" = NA, "0.1" = NA)
  result_AB_d_list = list("1" = NA, "0.5" = NA, "0.2" = NA, "0.15" = NA, "0.1" = NA)
  result_AB_d_r_list = list("1" = NA, "0.5" = NA, "0.2" = NA, "0.15" = NA, "0.1" = NA)
  result_C_ivw_list = list("1" = NA, "0.5" = NA, "0.2" = NA, "0.15" = NA, "0.1" = NA)
  result_C_adivw_list = list("1" = NA, "0.5" = NA, "0.2" = NA, "0.15" = NA, "0.1" = NA)
  result_C_GRAPPLE_list = list("1" = NA, "0.5" = NA, "0.2" = NA, "0.15" = NA, "0.1" = NA)
  iv_strength_list = c()
  parameters_list = c()
  ...
  for (weight_index in seq_along(sample_weight_list)) {
    ...
    iv_strength = min(eigen(solve(.sqrt_matrix(parameters$Sigma_X)) %*% parameters$VX_tilde %*% solve(.sqrt_matrix(parameters$Sigma_X)))$values) * sqrt(2000) # Q 2025: times sqrt(pz) while presenting IV strength 
    ...
    bias_AB_matrix = bias_AB_d_matrix = bias_AB_d_r_matrix = 
      bias_C_ivw_matrix = bias_C_adivw_matrix = bias_C_GRAPPLE_matrix =
    ...
    for (i in 1:eloop) {
      ...
      C_GRAPPLE = simulation_result[[14]]
...
save(simulate_result, file = "results/simulate_result_250327_with_GRAPPLE.RData")
round(simulate_result$iv_strength_list, 3)


#### plot heatmap/ generating table ####

load("results/simulate_result_250327_with_GRAPPLE.RData")
...
plot_heatmap <- function(parameters_list,
                         ...
                         result_ivw_list, result_adivw_list, result_GRAPPLE_list,
                         weight_to_plot)
  {
  px <- 24
  py <- 10
  ...
  # mr.GRAPPLE
  abs_mean_entry_bias_GRAPPLE <- rep(NA, (px*py)) # this is for the heatmap
  mean_entry_bias_GRAPPLE <- rep(NA, (px*py)) # this is for reporting the avg bias
  sd_entry_bias_GRAPPLE <- rep(NA, (px*py))
  for (i in 1:(px*py)) {
    abs_mean_entry_bias_GRAPPLE[i] <- abs(as.numeric(mean(result_GRAPPLE_list[[weight_to_plot]][i,])))
    mean_entry_bias_GRAPPLE[i] <- mean(result_GRAPPLE_list[[weight_to_plot]][i,])
    sd_entry_bias_GRAPPLE[i] <- sd(result_GRAPPLE_list[[weight_to_plot]][i,])
  }
  ...
  # mr.GRAPPLE
  avg_bias_GRAPPLE <- matrix(abs_mean_entry_bias_GRAPPLE, nrow = px, ncol = py)
  avg_bias_GRAPPLE <- t(avg_bias_GRAPPLE)
  avg_bias_GRAPPLE <- as.data.frame(avg_bias_GRAPPLE)
  colnames(avg_bias_GRAPPLE) <- 1:px
  rownames(avg_bias_GRAPPLE) <- 1:py
  ...
  hm_bias_6 = pheatmap::pheatmap(avg_bias_GRAPPLE, breaks = breaks,
                     main = "mr.GRAPPLE estimator", fontsize = 8,
                     cluster_rows = FALSE, cluster_cols = FALSE,
                     color = colorRampPalette(c("white", "red"))(100),
                     height = 10,
                     width = 8, silent = TRUE)$gtable
  ...
  combined_hm_bias = gridExtra::grid.arrange(
    grobs = list(hm_bias_6, hm_bias_4, hm_bias_5, hm_bias_1, hm_bias_2, hm_bias_3),
    ncol = 6,
  ...
  # mr.GRAPPLE
  avg_sd_GRAPPLE <- sd_entry_bias_GRAPPLE
  avg_sd_GRAPPLE <- matrix(avg_sd_GRAPPLE, nrow = px, ncol = py)
  avg_sd_GRAPPLE <- t(avg_sd_GRAPPLE)
  avg_sd_GRAPPLE <- as.data.frame(avg_sd_GRAPPLE)
  colnames(avg_sd_GRAPPLE) <- 1:px
  rownames(avg_sd_GRAPPLE) <- 1:py
  ...
  hm_sd_6 = pheatmap::pheatmap(avg_sd_GRAPPLE, breaks = breaks_sd,
                     main = "mr.GRAPPLE estimator", fontsize = 8,
                     cluster_rows = FALSE, cluster_cols = FALSE,
                     color = colorRampPalette(c("white", "red"))(100),
                     height = 10,
  ...
  combined_hm_sd = gridExtra::grid.arrange(
    grobs = list(hm_sd_6,hm_sd_4, hm_sd_5, hm_sd_1, hm_sd_2, hm_sd_3),
    ncol = 6,
  ...
  weight_index = match(weight_to_plot, c("1", "0.5", "0.2", "0.15", "0.1"))
  ...
  mean_abs_bias_GRAPPLE = round(mean(abs(as.matrix(avg_bias_GRAPPLE))), 3)
  ...
  mean_sd_GRAPPLE = round(mean(as.matrix(avg_sd_GRAPPLE)), 3)
  ...
  median_abs_bias_GRAPPLE = round(median(abs(as.matrix(avg_bias_GRAPPLE))), 3)
  ...
  median_sd_GRAPPLE = round(median(as.matrix(avg_sd_GRAPPLE)), 3)
  ...
  quantile_abs_bias_GRAPPLE = round(quantile(abs(as.matrix(avg_bias_GRAPPLE)), c(0.25, 0.75)), 3)
  ...
  quantile_sd_GRAPPLE = round(quantile(as.matrix(avg_sd_GRAPPLE), c(0.25, 0.75)), 3)
  #####

  ggplot2::ggsave(sprintf("results/hmap_3.24_with_GRAPPLE/bias_hm_weight_%s.png",weight_to_plot), plot = combined_hm_bias, width = 12, height = 4)
  ggplot2::ggsave(sprintf("results/hmap_3.24_with_GRAPPLE/sd_hm_weight_%s.png",weight_to_plot), plot = combined_hm_sd, width = 12, height = 4)

  return(list(mean_abs_C_entry = mean_abs_C_entry,
              # report mean
              mean_abs_bias_GRAPPLE = mean_abs_bias_GRAPPLE, mean_sd_GRAPPLE = mean_sd_GRAPPLE,
              ...
              # report median
              median_abs_bias_GRAPPLE = median_abs_bias_GRAPPLE, median_sd_GRAPPLE = median_sd_GRAPPLE,
              ...
              # report quantile
              quantile_abs_bias_GRAPPLE = quantile_abs_bias_GRAPPLE, quantile_sd_GRAPPLE = quantile_sd_GRAPPLE,
...
plot_heatmap(parameters_list = simulate_result$parameters_list,
             ...
             result_GRAPPLE_list = simulate_result$result_GRAPPLE_list,
             weight_to_plot="0.1")



#### choose regularization rate \lambda ####
# c1: [1] 1.736270e-14 3.987294e-15 4.338602e-16 1.557404e-16 6.208223e-17
# c10: [1] 4.624813e-12 3.205456e-13 2.495843e-15 2.234604e-16 4.368767e-18
# c10,5,75: [1] 1.237409e-11 1.370038e-12 7.415542e-15 5.989321e-16 5.563996e-18
# c10,10,75: [1] 3.065489e-11 2.491197e-12 1.252192e-14 8.756005e-16 1.432915e-17
c=10
me_weight_list = c(1, 0.5, 0.2, 0.15, 0.1)
eloop = 10
...
for (i in 1:eloop){
  ...
  for (me_weight in me_weight_list){
    ...
    for (q in q_list) {
      ...
      regu_rate = 10 * sigma_y2^2 * exp(c * (q/sqrt(n)-(mu_min+1)))/n
...
#### IV from data: about 20 ####
# Mean F-statistic across SNPs
gamma_j = as.matrix(lip_data[,paste0('gamma_exp',1:24)])
var_Z = 2 * lip_data$eaf * (1 - lip_data$eaf) # each Z is sum of two alleles, so var(Z) = var(Z^1+Z^2) = 2*var(Z^1), where Z1, Z2 ~ Binomial(eaf.outcome)
gamma_j_adj = gamma_j / sqrt(var_Z)
se_j = as.matrix(lip_data[,paste0('se_exp',1:24)])
se_j_adj = se_j / sqrt(var_Z)

# IV strength
# 每个 SNP 的 F 值（矩阵方式，元素平方除以标准误平方）
F_mat <- (gamma_j_adj)^2 / (se_j_adj)^2  # 114 x 24 矩阵

# 计算每个 exposure 的平均 F 值（按列求均值）
mean_F_by_exposure <- colMeans(F_mat)

# 总体平均 F 值（跨所有 exposure 和 SNP）
mean_F_overall <- mean(F_mat)

Iv_strength = mean_F_by_exposure
...
.get_parameters_old = function(me_weight, r_RR = 5, px, r_approx = 5){  # original function without standard var z to 1
  # ## test
  # me_weight =1
  # r_RR = 5
  # px=24
...
run_simulation_without_GRAPPLE <- function(regularized = TRUE, regularization_rate_list){
  sample_weight_list = c(1, 0.5, 0.2, 0.15, 0.1)
  eloop = 100
  result_AB_list = list("1" = NA, "0.5" = NA, "0.2" = NA, "0.15" = NA, "0.1" = NA)
  result_AB_d_list = list("1" = NA, "0.5" = NA, "0.2" = NA, "0.15" = NA, "0.1" = NA)
  ...
  for (weight_index in seq_along(sample_weight_list)) {
    ...
    bias_AB_matrix = bias_AB_d_matrix = bias_AB_d_r_matrix = 
      bias_C_ivw_matrix = bias_C_adivw_matrix = 
...
plot_heatmap_without_GRAPPLE <- function(parameters_list,
                                         result_naive_MRrr_list, result_MRrr_list, result_r_MRrr_list,
                                         result_ivw_list, result_adivw_list,
                                         weight_to_plot)
{
  px <- 24
  py <- 10
  ...
  combined_hm_bias = gridExtra::grid.arrange(
    grobs = list(hm_bias_4, hm_bias_5, hm_bias_1, hm_bias_2, hm_bias_3),
    ncol = 5,
    top = grid::textGrob(sprintf("Absolute average bias by entry, weight = %s",weight_to_plot), gp = grid::gpar(fontsize = 16))
  )
  combined_hm_bias
  ...
  combined_hm_sd = gridExtra::grid.arrange(
    grobs = list(hm_sd_4, hm_sd_5, hm_sd_1, hm_sd_2, hm_sd_3),
    ncol = 5,
    top = grid::textGrob(sprintf("SD by entry, weight = %s",weight_to_plot), gp = grid::gpar(fontsize = 16))
  )
  combined_hm_sd
  ...
  ggplot2::ggsave(sprintf("results/hmap_3.27_without_GRAPPLE/bias_hm_weight_%s.png",weight_to_plot), plot = combined_hm_bias, width = 12, height = 4)
  ggplot2::ggsave(sprintf("results/hmap_3.27_without_GRAPPLE/sd_hm_weight_%s.png",weight_to_plot), plot = combined_hm_sd, width = 12, height = 4)
