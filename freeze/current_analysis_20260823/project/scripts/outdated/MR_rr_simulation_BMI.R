...
.get_parameters = function(me_weight, r_RR = 5, px, r_approx = 5){
  ...
  py = 8 # py can be changed to any number from 1 to 24
  ...
  Sigma_Xj = lapply(1:pz_tmp, function(j)
    diag(sigma_gamma_j[j,]) %*% Sigma %*% diag(sigma_gamma_j[j,]))
...
save(simulate_result, file = "results/simulate_result_bmi_250110_1000_1e13.RData")

load("results/simulate_result_bmi_250110_1000_1e13.RData")
