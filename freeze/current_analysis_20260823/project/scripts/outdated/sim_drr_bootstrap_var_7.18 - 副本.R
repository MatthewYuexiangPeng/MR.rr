...
# estimate V_X_tilde
V_X_tilde_star = t(x_j_hat) %*% x_j_hat / px - Sigma_X_hat
C_star = AB_d_hat
...
for (i in 1:(px*py)){
  C_drr_hat_entrywise_bt_list[[i]] = rep(0, bootstrap_loop)
}
for (i in 1:bootstrap_loop){
...
for (i in 1:(px*py)){
  C_drr_hat_entrywise_bt_list_2[[i]] = rep(0, bootstrap_loop)
}
for (i in 1:bootstrap_loop){
  # sample bt_sample_number rows from x_j_hat, y_j_hat
  index_bt = sample(1:pz, bt_sample_number, replace = TRUE)
  x_j_hat_star_2 = x_j_hat_star[index_bt,]
  y_j_hat_star_2 = y_j_hat_star[index_bt,]
...
for (i in 1:(px*py)){
  C_drr_hat_entrywise_bt_95_list_2[[i]] = quantile(C_drr_hat_entrywise_bt_list_2[[i]], c(0.025, 0.975))
}
# see whether the true C is in the 95% percentile interval
...
#### percentile coverage: 0.48
#### CI coverage: 0.2
### non-parametric bootstrap:
#### percentile coverage: 0.1925
#### CI coverage: 0.07

## un-penalized estimator:
### parametric bootstrap:
#### percentile coverage: 0.4525
#### CI coverage: 0.16
### non-parametric bootstrap:
#### percentile coverage: 0.1825
#### CI coverage: 0.0675
