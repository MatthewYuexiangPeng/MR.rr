...
mr_rr_sparse <- function(GAMMA_hat, gamma_hat, W, Sigma_X, lambda = rep(1e-3, ncol(gamma_hat)), r = 2, max_iter = 100, tol = 1e-2) {
  ...
  return(list(A = A_hat, B = B_hat, AB = C_hat, B_raw = B_raw, AB_raw = C_raw))
}
...
#### ivw estimator on each outcome (parallel)#### 
# # NOTE: in our simulation, since we simulate data on Sigma_X and Sigma_Y, we can not use the real SEs to perform IVW, instead we can only use Sigma_X and Sigma-Y to approximate the SEs.This may lead the IVW perform very bad
# # We can not simply use the real se and beta, becausde in the real data, we don't know true C, and we can not compare ivw with the mrRR methods.
# ivw_multiple_outcomes = function(Y, X, Sigma_X, Sigma_Y, n_cores = parallel::detectCores() - 1) {
#   pz = nrow(Y)
#   px = ncol(X)
#   py = ncol(Y)
#   
...
#   
#   # Parallel loop
#   result_list = foreach(i = 1:py, .packages = c("mr.divw")) %dopar% {
#     res = mvmr.ivw(
#       beta.exposure = beta.exposure,
#       se.exposure = se.exposure,
#       beta.outcome = as.vector(beta.outcome[, i]),
#       se.outcome = se.outcome[, i]
#     )
#     return(t(res$beta.hat))  # 1 x px row
#   }
#   
...
#### adIVW estimator on each outcome (parallel)####
# adivw_multiple_outcomes = function(Y, X, Sigma_X, Sigma_Y, n_cores = parallel::detectCores() - 1) {
#   pz = nrow(Y)
#   px = ncol(X)
#   py = ncol(Y)
#   
...
#       phi_cand = NULL
#     )
#     return(t(res$beta.hat))  # row vector (1 x px)
#   }
#   
#   stopCluster(cl)  # cleanup
#   
#   result_matrix = do.call(rbind, result_list)
#   
#   return(result_matrix)
...
# help(mr.raps)
# Q 2025: problem: we don't have access to true se_j and beta_j in simulation, so we are not applying these methods that require these info "fairly"

# mr.raps_multiple_outcomes = function(Y, X, Sigma_X, Sigma_Y) {
#   pdf(NULL)
#   on.exit(dev.off(), add = TRUE)
#   
#   # 禁止 console 输出（包括 print, cat, message, warning）
...
#           se.outcome <- matrix(rep(se.outcome_row, each = pz), nrow = pz, byrow = TRUE)
#           
#           result_matrix <- matrix(NA, nrow = outcome_num, ncol = exposure_num)
#           for (i in 1:outcome_num) {
#             for (j in 1:exposure_num) {
#               df <- data.frame(
#                 beta.exposure = beta.exposure[, j],
#                 se.exposure = se.exposure[, j],
#                 beta.outcome = beta.outcome[, i],
#                 se.outcome = se.outcome[, i]
#               )
#               result_matrix[i, j] <- mr.raps(df)$beta.hat
#             }
#           }
#           
#         }, file = tempfile())  # 控制台输出重定向到临时文件
#       )
#     )
#   )
#   
...
#           
#           pz <- nrow(Y)              # SNP 数量
#           outcome_num <- ncol(Y)       # outcome 数量
#           exposure_num <- ncol(X)      # exposure 数量
#           
#           # 构造标准误矩阵
#           se_exposure_row <- t(as.matrix(sqrt(diag(Sigma_X))))
#           se_exposure <- matrix(rep(se_exposure_row, each = pz), nrow = pz, byrow = TRUE)
#           se_outcome_row <- sqrt(diag(Sigma_Y))
#           se_outcome <- matrix(rep(se_outcome_row, each = pz), nrow = pz, byrow = TRUE)
#           
#           # 并行计算设置：检测可用核心数，并设置并行集群
#           library(foreach)
#           library(doParallel)
...
#           registerDoParallel(cl)
#           
#           # 利用 foreach 对每个 outcome 并行计算（每个任务内部遍历所有 exposure）
#           result_matrix <- foreach(i = 1:outcome_num, .combine = rbind, 
#                                    .packages = "mr.raps") %dopar% {
#                                      res_row <- numeric(exposure_num)
#                                      for (j in 1:exposure_num) {
#                                        df <- data.frame(
#                                          beta.exposure = X[, j],
#                                          se.exposure   = se_exposure[, j],
#                                          beta.outcome  = Y[, i],
#                                          se.outcome    = se_outcome[, i]
#                                        )
#                                        # 调用 mr.raps 包中的 mr.raps 函数
#                                        res <- mr.raps::mr.raps(df)
#                                        res_row[j] <- res$beta.hat
#                                      }
#                                      res_row
#                                    }
#           
#           stopCluster(cl)
#           
#           # 将结果存入全局变量（供后续 capture.output 返回）
#           result_matrix_global <<- result_matrix
#         }, file = tempfile())
#       )
#     )
#   )
#   
#   # 返回结果
#   return(result_matrix_global)
# }


#### GRAPPLE:grappleRobustEst ####
# library(devtools)
# install_github("jingshuw/grapple")
# GRAPPLE_multiple_outcomes = function(Y, X, Sigma_X, Sigma_Y, n_cores = parallel::detectCores() - 1) {
#   pz = nrow(Y)
#   px = ncol(X)
#   py = ncol(Y)
#   
...
#   
#   # Run foreach loop
#   AB_list = foreach(k = 1:py, .packages = c("GRAPPLE")) %dopar% {
#     df = data.frame(
#       beta.exposure,
#       se.exposure,
#       beta.outcome[, k, drop = FALSE],
#       se.outcome[, k, drop = FALSE],
#       SNP = 1:pz
#     )
#     
#     colnames(df)[1:px] = paste0("gamma_exp", 1:px)
#     colnames(df)[(px + 1):(2 * px)] = paste0("se_exp", 1:px)
#     colnames(df)[2 * px + 1] = "gamma_out1"
#     colnames(df)[2 * px + 2] = "se_out1"
#     
#     res = grappleRobustEst(data = df, plot.it = FALSE)
#     return(res$beta.hat)
#   }
#   
#   stopCluster(cl)  # cleanup
#   
#   # Combine results
#   AB_matrix = do.call(rbind, AB_list)
#   
#   return(AB_matrix)
# }
...
# GRAPPLE_multiple_outcomes = function(Y, X, Sigma_X, Sigma_Y) {
#   pz = nrow(Y)
#   px = ncol(X)
#   py = ncol(Y)
#   
...
#   se.outcome = matrix(rep(se.outcome_row, each = pz), nrow = pz, byrow = TRUE)
#   
#   AB_matrix = matrix(NA, nrow = py, ncol = px)
#   
#   for (k in 1:py) {
#     df = data.frame(
#       beta.exposure,
