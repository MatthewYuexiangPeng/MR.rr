...
.simulation = function(parameters, regularized = TRUE, regularization_rate = 1e-13, pz) {
  n = pz
...
.simulation_sparse_compare <- function(
    parameters,
    B_true,
    lambda,
    regularization_rate,
  ...
  res_regularized <- mr_rr_regularized(GAMMA_hat, gamma_hat, r = r_RR, W = W, Sigma_X = Sigma_X, regularization_rate = regularization_rate)
  
  # --- Step 5: Sparse MR-RR ---
  res_sparse <- mr_rr_sparse(GAMMA_hat, gamma_hat, W = W, Sigma_X = Sigma_X, lambda = lambda, r = r_RR, max_iter = 100)
  
  # Resolve pathway permutation and sign ambiguity
  ...
  res_sparse$AB <- aligned$A %*% aligned$B
...
#### set eta and regularization rate ####
me_weight_sparse     <- 1
effect_weight_sparse <- 1

sparse_lambda <- rep(1e-3, px)

regularization_rate_sparse <- 3.105502e-15

support_threshold <- 1e-2

#### 260719 simulation & plots ####
run_simulation_sparse_compare <- function(
    C,
    B_sparse,
    me_weight,
    effect_weight,
  ...
  B_hat_bin_mat <- apply(result_B_sparse, 2, function(col) as.numeric(abs(col) >= support_threshold))
...
res <- run_simulation_sparse_compare(
  C = C,
  B_sparse = B_sparse,
  me_weight = me_weight_sparse,
  effect_weight = effect_weight_sparse,
  lambda = sparse_lambda,
  regularization_rate = regularization_rate_sparse,
  r_RR = 2,
  eloop = 1000,
  pz = pz,
  support_threshold = support_threshold
...
save(
  ...
  support_threshold,
  file = "results/simulation_sparseB_me1_effect1_eta_1e-3_260719.RData"
)

load("results/simulation_sparseB_me1_effect1_eta_1e-3_260719.RData")
...
# recovery rate
compute_sparse_support_metrics <- function(res, threshold = 1e-3) {
  B_true <- res$B_sparse                  # r x px
...
metrics <- compute_sparse_support_metrics(
  res,
  threshold = res$support_threshold
)
