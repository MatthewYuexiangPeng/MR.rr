...
.get_parameters = function(C, me_weight, effect_weight, r_RR = 2, pz){
  # ## test
  # me_weight =1
  # r_RR = 2
  # px=9
  ...
  # var_Z & VX_tilde ----------------------------------------------------------------
  # pz = 177 # pz can be changed to any number
  px = 9
  ...
  # updated 260716 dont need resample pz since we set pz=177
  z_index = seq_len(177)
  ...
  VX_tilde <- .second_moment(gamma_j_star_temp) - Sigma_X
...
# fix rank =2
.simulation <- function(parameters, regularization_rate = 1e-13, pz) {
  n = pz
  py = parameters$py
  px = parameters$px
...
# ============================================================ #
# generate C in regular way
gen_C = function(px, py, r){
  C = matrix(rnorm(py*px), py, px)
  U = svd(C)$u
  V = svd(C)$v
  C = U %*% diag(c(rep(1, r),
                   rep(0, min(px,py)-r))) %*% t(V)
  return(C)
}


# generate C with sparse B
gen_C_sparseB = function(px, py, r, lip_data_real){
  ...
  return(list(C = C, B_sparse = B_sparse))
}
...
# sim setting 1. generate C in regular way
C = gen_C(px, py, r)
round(C, 3)

# sim setting 2. generate C with sparse B
# temp = gen_C_sparseB(px, py, r, lip_data_real)
# C = temp$C
# B_sparse = temp$B_sparse
# round(C, 3)
# round(B_sparse, 3)
...
Y = C %*% X
round(X,3)
# -0.560 -0.230  1.559  0.071  0.129  1.715  0.461 -1.265 -0.687

# calculate population level SIV
set.seed(123)
calc_population_SIV <- function(parameters, pz) {
  Sigma_X <- parameters$Sigma_X
  Sigma_gg <- parameters$VX_tilde
  
  Sigma_X_inv_sqrt <- solve(.sqrt_matrix(Sigma_X))
...
for (me_weight in me_weight_list) {
  for (effect_weight in effect_weight_list) {
    ...
    siv <- calc_population_SIV(par, pz)
    
    cat(
      "me =", me_weight,
      "; effect =", effect_weight,
      "; population SIV =", round(siv, 2),
      "\n"
    )
  }
}

# same in two C settings:
# me = 2.5 ; effect = 0.25 ; population SIV = 3.6 
# me = 2.5 ; effect = 1 ; population SIV = 14.41 
# me = 1 ; effect = 0.25 ; population SIV = 9 
# me = 1 ; effect = 1 ; population SIV = 36.02 
...
for (i in 1:eloop){
  ...
  for (me_weight in me_weight_list){
    for (effect_weight in effect_weight_list){
      ...
      parameters = .get_parameters(C, me_weight=me_weight, effect_weight=effect_weight, r_RR = 2, pz)
      n = pz
      py = parameters$py
      px = parameters$px
      ...
      Sigma_hgammahgamma_hat = .second_moment(x_j_hat)
...
# updated 260716
# setting 1: 1.007845e-10 2.340371e-12 1.578970e-12 3.103420e-15
# setting 2: 9.109067e-11 2.340371e-12 1.578970e-12 3.105502e-15

# note: this in in order (0.25 2.5) (0.25 1) (1 2.5) (1 1). the final output for paper should switch index 2 and 3. This is already handled in code.
...
# setting 1:
save_sim_pred_filename = "results/simulate_result_pred_260717_regularC.RData"

# setting 2:
# save_sim_pred_filename = "results/simulate_result_pred_260718_sparseC.RData"

run_simulation_prediction <- function(regularization_rate_list, 
                                      eloop = 500, 
                                      pz){
  ...
  for (me_weight_index in seq_along(me_weight_list)){
    ...
    for (effect_weight_index in seq_along(effect_weight_list)){
      ...
      parameters = .get_parameters(C, me_weight, effect_weight, r_RR = 2, pz)
      C = parameters$C
      A = parameters$A
      B = parameters$B
      A_d = parameters$A_d
      ...
      iv_strength = min(eigen(solve(.sqrt_matrix(parameters$Sigma_X)) %*% parameters$VX_tilde %*% solve(.sqrt_matrix(parameters$Sigma_X)))$values) * sqrt(pz)
...
# round(simulate_result_prediction$iv_strength_list, 2) # this is sample SIV, not stable. use population SIV instead
...
# ============================================================ #
# 260506 use M0 test
run_rank_selection_sim <- function(parameters, eloop = 300, pz) {
  n  = pz
  py = parameters$py
  px = parameters$px
  VX_tilde = parameters$VX_tilde
...
for (idx in seq_along(setting_names)) {
  ...
  rank_results_all[[sim_name]] <- run_rank_selection_sim(params, eloop = 1000, pz)
}

# setting 1:
# save(rank_results_all, file = "results/simulate_result_rank_260717_regularC.RData")

# setting 2:
save(rank_results_all, file = "results/simulate_result_rank_260718_sparseC.RData")

load("results/simulate_result_rank_260717_regularC.RData")
load("results/simulate_result_rank_260718_sparseC.RData")


# ============================================================ #
# 9.0 Select eta for sparse MR-rr by visual inspection (can only run once to choose eta) ####
# ============================================================ #
# ------------------------------------------------------------ #
# Generate one simulated dataset
# ------------------------------------------------------------ #
.generate_sparse_eta_data <- function(parameters, pz) {
  ...
  # True SNP-exposure and SNP-outcome effects
  gamma_j_star <- MASS::mvrnorm(
    n     = pz,
    mu    = rep(0, px),
    Sigma = VX_tilde,
    tol   = 100
  ...
  # Estimated GWAS associations
  x_j_hat <- gamma_j_star +
    MASS::mvrnorm(
      n     = pz,
      mu    = rep(0, px),
      Sigma = Sigma_X,
...
# ------------------------------------------------------------ #
# Calculate eta paths for all simulation settings
# ------------------------------------------------------------ #
select_sparse_eta_simulation <- function(
    C_base,
    me_weight_list,
    effect_weight_list,
    pz,
    eta_grid = eta_grid <- 10^seq(-5, -1, length.out = 10),
    pilot_reps = 20,
    zero_tol = 1e-2,
    max_iter = 100,
    algorithm_tol = 1e-2,
  ...
  for (me_weight in me_weight_list) {
    for (effect_weight in effect_weight_list) {
      ...
      sim_name <- paste0(
        "me_", me_weight,
        "_effect_", effect_weight
      )
      
      cat("\nSelecting eta for:", sim_name, "\n")
      ...
      for (rep_index in seq_len(pilot_reps)) {
        
        # Generate one dataset and keep it fixed across all eta
        sim_data <- .generate_sparse_eta_data(
          parameters = parameters,
          pz = pz
        )
        
        x_j_hat <- sim_data$x_j_hat
        y_j_hat <- sim_data$y_j_hat
        ...
        for (eta_index in seq_along(eta_grid)) {
          ...
          if (is.null(fit_eta) ||
            ...
            path_list[[result_index]] <- data.frame(
              sim_name     = sim_name,
              me_weight    = me_weight,
              effect_weight = effect_weight,
              pilot_rep    = rep_index,
          ...
          # Use raw output if mr_rr_sparse returns it
          ...
          C_hat <- if (!is.null(fit_eta$AB_raw)) {
            fit_eta$AB_raw
          } else if (!is.null(fit_eta$AB)) {
            fit_eta$AB
          } else {
            fit_eta$A %*% B_hat
          }
          
          # Same debiased objective used for selecting phi
          # Do not include eta * ||B||_1
          residual_term <- norm(
            (y_j_hat - x_j_hat %*% t(C_hat)) %*% W_sqrt,
            type = "F"
          )^2 / pz
          ...
          n_zero    <- sum(abs(B_hat) < zero_tol)
          n_nonzero <- sum(abs(B_hat) >= zero_tol)
          zero_prop <- n_zero / length(B_hat)
  ...
  path_raw <- bind_rows(path_list)
  
  # Average over pilot simulation replicates
  path_summary <- path_raw %>%
    group_by(
      sim_name,
      me_weight,
      effect_weight,
    ...
    summarise(
      objective       = mean(objective, na.rm = TRUE),
      objective_sd    = sd(objective, na.rm = TRUE),
      zero_prop       = mean(zero_prop, na.rm = TRUE),
      n_zero          = mean(n_zero, na.rm = TRUE),
      n_nonzero       = mean(n_nonzero, na.rm = TRUE),
      success_rate    = mean(success),
      .groups         = "drop"
    ) %>%
    group_by(sim_name) %>%
    mutate(
      objective_min = min(objective, na.rm = TRUE),
      objective_increase = objective - objective_min,
      objective_increase_pct =
        100 * objective_increase /
        pmax(abs(objective_min), .Machine$double.eps)
    ) %>%
    ungroup()
  ...
  combined_plot <- p_objective / p_sparsity
  
  list(
    raw = path_raw,
    summary = path_summary,
    objective_plot = p_objective,
    sparsity_plot = p_sparsity,
    plot = combined_plot
  )
}

set.seed(123)
eta_selection_result <- select_sparse_eta_simulation(
  C_base            = C,
  me_weight_list    = me_weight_list,
  effect_weight_list = effect_weight_list,
  pz                = pz,
  eta_grid          = sort(unique(c(
    10^seq(-5, -1, length.out = 30),
    1e-3
  ))),
  pilot_reps        = 3,
  zero_tol          = 1e-2,
...
eta_selection_result$plot


print(
  eta_selection_result$summary %>%
    select(
      sim_name,
      eta,
      objective,
...
ggsave("results/eta_selection_plot_260729_regular_C.png", plot = eta_selection_result$plot, width = 10, height = 10, dpi = 300)

save(eta_selection_result, file = "results/eta_selection_result_260729_regular_C.RData") 
save(eta_selection_result, file = "results/eta_selection_result_260716_sparseB_C.RData") 
# load("results/eta_selection_result_260716_sparseC.RData")
...
set.seed(123)
.simulation_sparse <- function(parameters, sparse_lambda = rep(1e-3, 9), pz) {
  n = pz
  ...
  for (me_weight_index in seq_along(me_weight_list)) {
    ...
    for (effect_weight_index in seq_along(effect_weight_list)) {
      ...
      iv_strength = min(eigen(solve(.sqrt_matrix(parameters$Sigma_X)) %*% 
                                parameters$VX_tilde %*% 
                                solve(.sqrt_matrix(parameters$Sigma_X)))$values) * sqrt(pz)
      iv_strength_list = c(iv_strength_list, iv_strength)
      parameters_list = c(parameters_list, list(c(parameters, iv_strength = iv_strength)))
...
sparse_lambda_list = list(
  "me_1_effect_1"        = rep(1e-3, 9),
  "me_1_effect_0.25"     = rep(1e-3, 9),
  "me_2.5_effect_1"      = rep(1e-3, 9),
  "me_2.5_effect_0.25"   = rep(1e-4, 9)
)


sparse_results = run_simulation_sparse_prediction(
  regularization_rate_list = regularization_rate_list,
  sparse_lambda_list = sparse_lambda_list,  
  eloop = 1000,
  pz
)

# setting 1:
save(sparse_results, file = "results/simulate_result_pred_sparse_260717_regularC.RData")

# setting 2:
# save(sparse_results, file = "results/simulate_result_pred_sparse_260718_sparseC.RData") 

load("results/simulate_result_pred_sparse_260717_regularC.RData")
# load("results/simulate_result_pred_sparse_260718_sparseC.RData")
...
plot_simulation_result <- function(sim_result = simulate_result_prediction,
  ...
  full_path <- file.path(
    ...
    sprintf("lessvar_plot_%s_%s_generic_labels_sparseB_C.png", sim_name, today_str)
  )
...
load("results/simulate_result_pred_260717_regularC.RData")

# load("results/simulate_result_pred_260718_sparseC.RData")
...
plot_simulation_result_pred <- function(sim_result, 
  ...
  today_str <- format(Sys.Date(), "%y%m%d")
  filename  <- sprintf(
    "results/pred_plot_with_sparse_%s_type_%s_%s_regular_C.png",
    # "results/pred_plot_with_sparse_%s_type_%s_%s_sparseB_C.png",
                      ...
                      sparse_result = NULL)
{
  ...
  result_srivw_list      <- simulate_result_prediction$result_C_adivw_list
  ...
  me_weight_index     <- match(as.character(me_weight_to_plot), as.character(me_weight_list))
  effect_weight_index <- match(as.character(effect_weight_to_plot), as.character(effect_weight_list))
  ...
  s_srivw  <- compute_entry_stats(result_srivw_list)
  ...
  make_row <- function(estimator, stats) {
    bias_s <- summarize_vec(stats$abs_mean)
    sd_s   <- summarize_vec(stats$sd)
  ...
  out <- rbind(
    make_row("IVW",         s_ivw),
    make_row("SRIVW",       s_srivw),
    make_row("Naive MR-rr", s_naive),
    make_row("MR-rr",       s_d),
    make_row("Reg. MR-rr",  s_d_r)
  )
  
  if (!is.null(s_sparse)) {
    out <- rbind(
      out,
      make_row("Sparse MR-rr", s_sparse)
    )
  }
  
  out <- rbind(
    out,
    make_row("MrDAG", s_MrDAG)
  )
...
for (re in effect_weight_list) {
  for (me in me_weight_list) {
    cat("\n====================================\n")
    cat("me_weight =", me, "; effect_weight =", re, "\n")
    cat("====================================\n")
